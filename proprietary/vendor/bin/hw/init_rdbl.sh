...
HEADER_SIZE_BYTES=20480 # 0x5000 in hex
# 0x463C in hex according to
# https://docs.google.com/document/d/1H3wo7eRh_mz-j_7Vwaf9J47waouBeIEQlbtjcH9Z784/edit?resourcekey=0-CObA4wWZ3iOzgRvvHTm_mA&tab=t.0#bookmark=id.fuqlge3tqy8y
DELEGATE_HEADER_OFFSET=17980
# 0x4 in demical according to SW RollbackInfo filed in
# https://docs.google.com/document/d/1H3wo7eRh_mz-j_7Vwaf9J47waouBeIEQlbtjcH9Z784/edit?resourcekey=0-CObA4wWZ3iOzgRvvHTm_mA&tab=t.0#heading=h.ju4gw8bysuj3
...
if [[ "$rdbl_src_hash" == "$rdbl_dst_hash" ]]; then
    ...
    rdbl_src_ar_dec=$(get_antirollback_value "$RDBL_SRC" $DELEGATE_HEADER_OFFSET+$SW_ROLLBACK_INFO_OFFSET $VALUE_SIZE_BYTES)
    rdbl_dst_ar_dec=$(get_antirollback_value "$RDBL_DST" $DELEGATE_HEADER_OFFSET+$SW_ROLLBACK_INFO_OFFSET $VALUE_SIZE_BYTES)
